...
if st.session_state.is_boss:
        ...
        for u, u_data in reg.items():
            ...
            for idx, act in enumerate(list(pend)):
                with st.expander(f"{act['type']} - {u} (₱{act.get('amount',0):,.2f})"):
                    if 'details' in act:
                        st.warning(f"DETAILS: {act['details']}")
                    c1, c2 = st.columns(2)
    ...
    with t2:
        ...
        audit_data = [{"NAME": n, "PIN": i.get('pin'), "WALLET": i.get('wallet'), "REF": i.get('ref_by')} for n, i in reg.items()]
        st.table(audit_data)
    ...
    for idx, item in enumerate(list(active_inv)):
        ...
        with st.container():
            ...
            with cb:
                ...
                elif is_op: st.success("🔓 WINDOW OPEN")
